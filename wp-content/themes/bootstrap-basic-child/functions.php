...
function remove_scripts() {
	$parent_style = 'bootstrap-basic-style';
...
function childThemeEnqueueScripts() {
	...
	wp_enqueue_style('bootstrap-basic-child-style', get_stylesheet_directory_uri() . '/style.css', array( $parent_style ), wp_get_theme()->get('Version') );
...
if (!isset($bootstrapbasic_sidebar_left_size)) {
	$bootstrapbasic_sidebar_left_size = apply_filters('bootstrap_basic_column_left', 3);
}
// Right sidebar column size.
if (!isset($bootstrapbasic_sidebar_right_size)) {
	$bootstrapbasic_sidebar_right_size = apply_filters('bootstrap_basic_column_right', 4);
}
