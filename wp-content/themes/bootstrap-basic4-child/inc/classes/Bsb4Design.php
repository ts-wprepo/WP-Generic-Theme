...
if (!class_exists('\\BootstrapBasic4\\Bsb4Design')) {
    ...
    class Bsb4Design
    {
        ...
        public function attachment($return = false)
        {
            ...
            switch (strtolower($type)) {
                ...
                case 'video/x-msvideo':
                    ...
                    break;
                default:
                    $output = '<div class="card"><div class="card-body"><i class="fa fa-download"></i> ' . wp_get_attachment_link() . '</div></div>';
                    break;
            }// endswitch;
            unset($type);
        ...
        {
            $output = sprintf('<span class="categories-icon fa fa-folder-open" title="' . __('Posted in', 'bootstrap-basic4') . '"></span> %1$s', $categories_list);
        ...
        {
            $comment_icon = '<i class="comment-icon fa fa-comment"></i> <span class="comment-total">%d Comment</span>';
            $comments_icon = '<i class="comment-icon fa fa-comments"></i> <span class="comment-total">%s Comments</span>';
                ...
                echo '" class="comment-body media">';
                    ...
                    echo '<footer class="comment-meta media-left">';
                        if (0 != $args['avatar_size']) {
                            echo get_avatar($comment, $args['avatar_size'], false, false, array( 'class' => 'comment-avatar' ) );
                        }
                    ...
                    echo '<div class="comment-content media-body">';
                        echo '<div class="comment-author vcard">';
                            ...
                            edit_comment_link('<span class="fa fa-edit "></span>' . __('Edit', 'bootstrap-basic4'), '<span class="edit-link">', '</span>');
        ...
        public function editPostLink($return = false)
        {
            ...
            if ($edit_post_link != null) {
                $edit_btn = '<a class="post-edit-link btn btn-light btn-sm" href="'.$edit_post_link.'" title="' . __('Edit', 'bootstrap-basic4') . '" role="button"><i class="edit-post-icon fa fa-edit" title="' . __('Edit', 'bootstrap-basic4') . '"></i></a>';
        ...
        public function postOn($return = false)
        {
            ...
            $output = sprintf(__('<span class="posted-on"><span class="fa fa-calendar"></span> %1$s</span><span class="byline"><span class="fa fa-user"></span>  %2$s</span>', 'bootstrap-basic4'),
                sprintf('<span>%3$s</span>',
                    esc_url(get_permalink()),
                    esc_attr(get_the_time()),
                    $time_string
                ),
                sprintf('<span class="author vcard"><span>%3$s</span></span>',
