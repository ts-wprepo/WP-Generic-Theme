    ...
    class BootstrapBasic4WalkerNavMenu extends \Walker_Nav_Menu
    {
        ...
        public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) 
        {
            ...
            if (isset($item->classes) && is_array($item->classes) && in_array('dropdown-divider', $item->classes)) {
                ...
                if ($depth <= 0) {
                    $attributes .= (is_object($args) && $args->has_children) ? ' class="nav-link ' . $a_class_names . '" aria-haspopup="true" aria-expanded="false"' : ' class="nav-link ' . $a_class_names . '"';// data-toggle="dropdown"
                } else {
